...
export function TableSuppliersClients({
  data,
  setOpenRegister,
  setDataSelect,
  setAction,
  ...
  const { deleteSuppliersClients } = useSuppliersClientsStore();
  function deleteCategory(p) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await deleteSuppliersClients({ id: p.id });
      }
    });
  }
  function edit(data) {
  ...
  const columns = [
    ...
    {
      accessorKey: "national_identifier",
      header: "Documento Nacional",
      cell: (info) => <span>{info.getValue()}</span>,
      enableColumnFilter: true,
      filterFn: (row, columnId, filterStatuses) => {
        if (filterStatuses.length === 0) return true;
